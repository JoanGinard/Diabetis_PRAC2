...
El joc de dades conté 101766 observacions i 50 variables i es corresponen a dades de pacients ingressats a un Hospital i que padeixen diabetis, però aquesta afecció no és, en general, la causa de l'ingrés. Es a dir, el joc de dades fa seguiment d'un grup de pacients que a banda del motiu pirncipal de l'ingrés tenen diabetis.

Les dades procedeixen de diferents hopsital dels EUA i recollides entre els anys 1999 i 2008.
...
3. __race__: Origen ètnic del pacient: Caucàssic, Asiàtic, Africà-Americà, Hispà i altres.
4. __gender__: masculí, femení i desconegut/no-válid.
5. __age__: Edat agrupada en intervals de 10 anys.
6. __weight__: Pes del pacient en lliures.
...
12. __medical_specialty__: Especialitat del metge que atendrà al pacient: cardiologia, cirurgià...
...
19. __diag_1__, __diag_2__ i __diag_3__: Són tres atributs corresponents a diagnóstics del pacient, diag_1 seria el diagnòstic primari i els altres dos diagnóstics secundaris. Estan codificats segons uns codis estandarditzats.
...
23. __Conjunt de MEDICACIONS__: Al dataset ara tenim tot un seguit de medicacions (24) relacionades amb la diabetes i on figura si la dosi es puja, es maté o es baixa o si no es pren el medicament. Com veurem més endavant la major part de medicacions apenes si tenen variació i en molts casos els pacients no en prenen.
...
Els diagnòstics (atributs *diag_1*, *diag_2* i *diag_3*) estan codificats per tres xifres (i si és necessari un punt i una o dues xifres més) segons un manual de codficació estàndar i que per exemple es pot consultar [aquí](https://www.mscbs.gob.es/estadEstudios/estadisticas/docs/Manual_de_codificacion.pdf)
...
Recordem que el nostre objectiu es tractar de predir el possible reingrés dels pacients del joc de dades a partir de les diferents dades que tenim.

Donada l'enorme quantitat de variables que tenim està clar que haurem de realitzar una selecció dels atributs a considerar però aquesta selecció s'haurà de realitzar durant el procés de neteja de les dades, donat que, com veurem, atributs que *a priori* resulten molt importants com el pes estan bàsicament buits. No obstant ja podem veure que dades com l'identificador del pacient o de la visita/ingrés no resultaran interessants, ni tampoc l'assegurança del pacient.

També farem constrat d'hipòtesis per veure si determinats factors com el sexe o la medicació de la diabetes peresenten una proporció major de reingrés.
...
El pes és un atribut amb un 96% de valors buits, cosa que resulta molt estranya perquè la diabetis en adults està molt associada a l'excés de pes i l'obesitat. No obstant l'article original que estudia el joc de dades (Strack et al. (2014), veure bilbiografia) assenyala que fins al 2009 els hospitals no estaven obligats a recollir aquesta dada en un format estructurat. Podria haver estat un atribut important però haurem de prescindir d'ell,

Mentre que a l'article esmentat es decideix substituir els valors nuls de l'especialitat mèdica per un valor "desconegut", nosaltres considerem que són massa valors per fer això, ja que generaria una categoria amb "dret propi" per dir-ho d'alguna manera. 
També hem pensat que ni sabem ni podem saber si aquests valors es corresponen sobretot a unes poques especialitats o més aviat es distribuieixen més uniformement, per tant, al igual que el pes, tot i que podria ser una característica important, no podem mantenir la variable
...
Com en aquests valors falta el diagnóstic primari, però no els secundaris, no ens queda més que concloure que són valors perduts. Podríem fer hipòstesi sobre si les dades es van introduir malament i en realitat el diagnóstic seucndari és el primari, però són tan pocs valors que simplement eliminem els registres corresponents
...
Els diagnòstics secundaris poden estar presents o no, per tant que hagi valors buits no és un problema.
...
Al camp gènere tenim alguns valors que són desconeguts o invalids
...
En el joc de dades tenim moltes variables que gairebé no presenten variació sinó que la immensa majoria dels valors són el mateix.
...
Aquesta funció només ens lleva variables que és molt clar que no varien, per tant encara queden algunes variables que varien molt poc, on la majoria dels valors són els mateixos.
...
A alguns atributs com a diagnòstics o tipus d'alta tenim un nombre molt elevat de categories que intentem reduir per tal que la informació quedi més agrupada i tingui més sentit
...
2. Els criteris en algun cas reflexen que estem tractant amb dades d'un sistema de salut diferent i resulta difícil entendre si les diferències entre els casos són importants (han de quedar) o si són més subtils i per tant no són necessàries per segons quins estudis.

</ol>

En qualsevol cas en la nostra opinió aquests dos camps tenen massa descriptors i han de ser molt més ser reduits (de fet no tots estan utilitzats a les dades). Per exemple gairebé tots els valors d'alta que no són "alta a casa" són tranferències del pacient a hospitals, residències, alta sota atencions d'enfermeres a casa i similars. Tots aquests valors han de quedar reduits a una sola categoria.

En algun cas hem de prendre decisions subjectives, per exemple, un alta voluntària ("against medical advice" o AMA) com la classifiquem? Són molt pocs casos i hem optat per juntar-la amb les altes però que no són "completes" es a dir que suposen tornar a l'hospital amb cites programades, amb cures externes...
...
Aprofitarem per crear dues variables binàries per si resulten d'utilitat en l'elaboració dels models. Una que ens doni 1 si el pacient és admès en menys de 30 i 0 en cas contrari i una altra que ens doni 1 si el pacient és readmés en menys d'un any (grup "<30" i ">30") i zero si no ho és.
...
Notem com sembla que la proporció de readmesos sembla la mateixa en homes que en dones. Aquesta serà una de les hipòtesis que comprovarem en el punt següent.
...
Com es pot apreciar les etines que diferent de blanca i negre tenen una presència més aviat testimonial. Juntarem totes aquestes i els valors desconeguts en la categoria "Other"
...
D'aquí tenim una altra possible hipotesi a comparar que és si la proporció de readmissió és major en alguna de les races.
...
Resulta molt curiós que aquest joc de dades sigui sobre pacients diabètics i la mesura de la glucosa l'haguem descartada (tenia variació gairebé zero) i la de la prova de l'hemobglobina glicosilada només s'hagi realitzat a un percentatge inferior al 20 %.


__CAMPS NUMÈRICS__


No representem ni retoquem els camps numèrics perquè, tot i que no tots els utilitzarem en l'elaboració dels models els podremm servir per a la comprovació de la normalitat de les dades al següent punt i també per a veure correlaciones entre les variables.
...
Representem les nostres variables numèriques abans de fer cap comprovació de normalitat, però abans esmentar que les variables tot i numèriques, no són continues sinó sencerest.
...
Com havíem anticipat les dues gràfiques es separen significativement de la línea en els extrems i per tant tampoc podríem considerar que aquestes variables segueixen una distribució normal.
...
Aprofitarem per veure si tenim c orrelació entre les variables numèriques, més dies a l'hospital impliquen més procediments? o més medicacions?
...
Fem un test d'hipotesis per comprovar si la proporció de dones que reingressen és superior la dels els homes amb un nivell de confiança del 95 %:
...
Per contra, pel cas d'un any, veiem que podem refusar la hipòtesi nul·la i concloure que la proporció de dones que reingressen és major.
...
Recordar que en tractar-se d'un estudi observacional no podem assignar una relació causa-efecte, per tant, ens hem d'abstenir d'assegurar res d'aquesta associació estadística.
...
Provarem ara un model de regressió logística que ens doni una probabilitat de ser readmes, en menys d'un mes i en menys d'un any.

__MODEL READMISSIÓ EN UN MES__

Abans de fer el model separem les mostres en una mostra d'entrenament i una altra de test fent servir la funció *createDataPartition* del paquet *caret*, que ja ens distribuix la variable a predir de forma equiibrada entre les dues mostres. Com tenim moltes dades ens reservem només el 65 % per l'entrenament
...
Per realitzar el model tenim moltes variables que podem incloure i s'hauria de fer una exploració entre les diferents combinacions i anar provant diferents interaccions entre variables. A la bilbiografia (James et al.) s'assenyala que hi ha 3 mètodes habituals: selecció cap endavant (*forward selection*, començar sense variables i anar afegint), selecció cap enrera (*backward selection*, el contrari que l'anterior cas) i seleccio barrejada (*mixed selection*, combinació del anteriors).

Nosaltres hem considerat que l'objectiu de la pràctica no era tant trobar el millor model com provar i triar els adequats al joc de dades, i que, per tant, l'exploració exhaustiva de les diferents combinacions quedava fora de l'abast de la pràctica. Hem fet algunes proves i ens quedem amb una combinació prou bona
...
Notem que el valor p es superior a 0.05 en gairebé tots els factors, cosa que segurament indica que el model no resulta gaire bó.

Veiem en una taula els resultats del model amb les dades del conjunt d'entrenament. Triem un valor del 50 % per decidir si classifiquem un resultat com postitiu o negatiu
...
_NOTEM_ com ja hem assenyalat que aquest model no ens aporta res. Si classifiquem tots els casos com de NO reingrés tendrem el mateix encert que el model.
...
Veiem que en aquesta ocasió tenim molts més coeficients amb p < 0.05, de fet gairebé tots. Notar com la variable sexe ara sí té un p <0.05, recordar que en el test d'hipòetis havíem vist com el percentatge de reingrés entre homes i dones era diferent pel cas d'un any.

Ara fem la taula amb els valors igual que abans, fixem un limit del 50 % en el criteri de separació.
...
Tot i que ja sabem que el model no és gaireb bó fem la representació de la corba roc fent servir *roc()* del paquet *pROC* i calculem l'àrea sota la corba
...
La corba està molt aprop de la diagonal indicant el que ja hem comentat de que el model no és gaire bó. I el valor de AUC és proper a 0.5

### ARBRES DE DECISIÓ - RANDOM FOREST

El model random Forest es basa en crear tot un conjunt d'arbres de decisió (d'aquí el nom de forest o bosc) i promitjar el resultat entre ells.
...
Veiem que l'error calculat és del 9.02%. (Molt breument, OOB o One out of Bag és un sistema en el qual les files de de les dades que no s'han fet servir en un arbre es fan servir com a test de l'arbre en qüestió i d'altres en les que no s'ha fet servir.)
...
La precisió que obtenim és semblant a la de la regresió logística i és semblant a la que obtendríem si decidissim classificar tots els casos com a no-readmesos, per tant, no guanyem gaire.

Com no es tracta d'un arbre sinó d'un sistema que fa servir diversos arbres, resulta difícil representar-ho gràficament, per això fem una representació de la importància de les diferents variables, segons el criteri de promig de decreixement en l'índex de GINI 


```{r}

# Becessitem reordenar les dades com un dataframe per poder fer la gràfica
...
Segons les dades tenim que les variables més importants són el nombre de proves de laboratori i de medicacions, amb bastant diferència.
...
__Bernadó, Ester__, "Disseny experimental en analítica de dades" (UOC)
